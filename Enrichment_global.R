...
###Visuallisation of differentially abundant MAGs - global-scaled
...
D2_kk_up_cyano <- enrichKEGG(gene = D2_geneUp_cyano,
                       ...
                       pAdjustMethod = "BH",
                       pvalueCutoff = 0.05)
D2_kk_up_df_cyano <- data.frame(D2_kk_up_cyano) %>% filter(category != "Human Diseases", Count > 5) %>% mutate(DA = "Up", Taxonomy = "Cyanobacteriota", Drought = "D2")
...
D2_kk_up_euk <- enrichKEGG(gene = D2_geneUp_euk,
                             ...
                             pAdjustMethod = "BH",
                             pvalueCutoff = 0.05)
D2_kk_up_df_euk <- data.frame(D2_kk_up_euk) %>% filter(category != "Human Diseases", Count > 5) %>% mutate(DA = "Up")
#No change up D2 euk
D2_kk_down_euk <- enrichKEGG(gene = D2_geneDown_euk,
                               organism     = 'ko',
                               pAdjustMethod = "BH",
                               pvalueCutoff = 0.05)
D2_kk_down_df_euk <- data.frame(D2_kk_down_euk) %>% filter(category != "Human Diseases", category != "Organismal Systems", Count > 5) %>% mutate(DA = "Down", Taxonomy = "Stramenopiles", Drought = "D2")
#Enrichement down D2 euk
...
D3_kk_up_hetero <- enrichKEGG(gene = D3_geneUp_hetero,
                           ...
                           pAdjustMethod = "BH",
                           pvalueCutoff = 0.05)
D3_kk_up_df_hetero <- data.frame(D3_kk_up_hetero) %>% filter(category != "Human Diseases", Count > 5) %>% mutate(DA = "Up")
#No change up D3 hetero
D3_kk_down_hetero <- enrichKEGG(gene = D3_geneDown_hetero,
                             organism     = 'ko',
                             pAdjustMethod = "BH",
                             pvalueCutoff = 0.05)
D3_kk_down_df_hetero <- data.frame(D3_kk_down_hetero) %>% filter(category != "Human Diseases", Count > 5) %>% mutate(DA = "Down")
#No change down D3 hetero

Enrichment_results <- rbind(D2_kk_up_df_cyano, D1_kk_down_df_euk, D2_kk_down_df_euk, D1_kk_up_df_hetero, D2_kk_up_df_hetero)
write.csv(Enrichment_results, "Results/Enrichment_results_GS.csv")
